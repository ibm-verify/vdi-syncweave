...
This is a simple IBM Security Verify Directory Integrator configuration that demonstrates the IBM Security Verify Directory Integrator Delta functionality.
...
The database is synchronized through the use of 3 JDBC Connectors respectively in AddOnly, Update and Delete modes. The Iterator Connector (IBM Security Verify Directory Integrator -> AssemblyLines -> "Deltas" -> "ReadTextFile" connector) using its delta database skips unchanged Entries and marks the others for add, delete or modify.
JavaScript code is placed in:
o IBM Security Verify Directory Integrator -> AssemblyLines -> Deltas -> "DB_INSERT" connector -> "Hooks" tab -> "AddOnly" sub-tab -> "Before Add" event
o IBM Security Verify Directory Integrator -> AssemblyLines -> Deltas -> "DB_UPDATE" connector -> "Hooks" tab -> "Update" sub-tab -> "Before Add" event
o IBM Security Verify Directory Integrator -> AssemblyLines -> Deltas -> "DB_UPDATE" connector -> "Hooks" tab -> "Update" sub-tab -> "Before update" event
o IBM Security Verify Directory Integrator -> AssemblyLines -> Deltas -> "DB_DELETE" connector -> "Hooks" tab -> "Delete" sub-tab -> "Before delete" event
...
IBM Security Verify Directory Integrator -> AssemblyLines -> "Deltas" AssemblyLine -> "ReadTextFile" connector -> click "Connection" -> set "File Path" value to examples/deltas/users1.txt -> click "Close".
4. Click "Run". 
...
IBM Security Verify Directory Integrator -> AssemblyLines -> "Deltas" AssemblyLine -> "ReadTextFile" connector -> click "Connection" -> set "File Path" value to examples/deltas/users2.txt -> click "Close".
7. Click "Run". 
