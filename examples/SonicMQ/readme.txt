...
1. Introduction
-----------------
This sample demonstrates how the SyncWeave JMS components (JMS Connector, System Queue) can use the SonicMQ server as a JMS provider.



1.1. SyncWeave JMS Drivers
----------------------
Since SyncWeave 6.1 is introduced the concept of SyncWeave JMS Drivers - these are modules which allow SyncWeave to use any JMS provider. Please note that these are different 
from the JMS provider drivers.
The SyncWeave JMS Driver for a particular provider deals with the specifics of obtaining a connection to the JMS provider (since according to the JMS 
specification this initialization is done in a provider-specific way).
The SyncWeave JMS driver implementations for the following JMS providers are called standard and are provided out of the box in SyncWeave:
	IBM WebSphere MQ
...
SyncWeave solution developers and SyncWeave bundlers can either
	...
	2.	write a custom SyncWeave JMS driver
		a.	either in Javascript using the SyncWeave JMS Script Driver (this is what this SonicMQ sample uses), or
		b.	in Java
...
1.2 The SonicMQ SyncWeave JMS Driver
--------------------------------
This SonicMQ sample uses the SyncWeave JMS Script Driver - a custom Javascript file (which is the implementation of this SyncWeave JMS driver for SonicMQ) is 
provided as part of this sample which allows the SyncWeave JMS component to use SonicMQ as a JMS provider.
...
2. Configuration
------------------
This custom SyncWeave Javascript Sonic MQ driver uses the following parameters:
...
For the JMS Connector these properties are taken from the Connector configuration in the SyncWeave config file.
...
Make sure the Sonic MQ driver sonic_jms_driver.js is located in the working folder of your SyncWeave installation.
For the examples to work, put the sonic_Client.jar file (you can find this in a Sonic MQ installation) in the jars folder of SyncWeave.
...
----------------------------
This example demonstrates how to use the Sonic MQ driver with the JMS Connector of SyncWeave.
...
Before running this example, SyncWeave System Queue must be properly configured. You should enter lines like the following in the global.properties/
solution.properties configuration file of SyncWeave:
...
tdi_store.jks is meant to be the JSSE keystore and truststore used by SyncWeave for the SSL communication.
...
The stores are created in such a way so that the message broker will trust SyncWeave and SyncWeave will trust the broker.
Before running the examples, the following properties should be entered in the global.properties/solution.properties configuration file of SyncWeave:
...
the �tdi� alias as SyncWeaveator side private key during the SSL communication.
...
tdi_store.jks description:
	purpose: JSSE keystore and truststore for SyncWeave
	contains SyncWeave private key under alias �tdi�
...
sonic_store.jks description:
	...
	contains SyncWeave certificate (in order to trust SyncWeave) under alias �tdi�
	password for the store is �secret�
...
the SSL_CIPHER_SUITES property in global.properties/solution.properties of SyncWeave (it is a property used internally by the Sonic client libraries). 
For example:
...
in the jars folder of SyncWeave.


Sonic MQ with JMS Connector using SSL
---------------------------------------
This example demonstrates how to use the Sonic MQ driver with the JMS Connector of SyncWeave using SSL.
...
The example uses the same SyncWeave configuration file as the example without SSL. The only difference is the way the System Queue is configured. To use SSL 
put the following properties in the global.properties/solution.properties file of SyncWeave:
