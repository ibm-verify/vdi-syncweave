...
/**
 * This class is a temporary class to convert an Entry to/from XML. The new hierachical Entry should
 * be used instead when it supports serialization.
 */
public class XML {
	
	@SuppressWarnings("unused")
	private static final String COPYRIGHT = com.ibm.di.server.CopyRight.OBJECT_CODE;
	private static ResourceHash sRes = ResourceHash.getHash("miserver");
	...
	public static Entry fromXML(Document document) throws Exception {
		...
		if(!root.getNodeName().equals("Entry"))
			throw new Exception(sRes.getString("Expected.Entry.tag", root.getNodeName()));
		...
		for(int i = 0; i < children.getLength(); i++) {
			...
			if(item.getNodeName().equals("Attribute")) {
				...
				if(target instanceof Entry)
					...
					System.out.println("Cannot set Property tag on " + target.getClass());
				...
				value = (item.getFirstChild() != null ? item.getFirstChild().getNodeValue() : null);
			}
			if(value == null)
				continue;
			...
			if(target instanceof Attribute)
				...
				System.out.println("Cant set " + value.getClass() + " in " + target.getClass());
		}
	}
