...
final class ConfigurationImpl implements Configuration {
	 ...
	 */
	ConfigurationImpl(UserRegistryConnector urc) throws ConfigurationException {
		super();
	...
	private File createFileFromUrl(String url) throws ConfigurationException {
		...
		if (!f.exists()) {
			...
			String msg = LogMessageHelper.getMsgResource().getMessage(
					LogMessageHelper.SAPR3_UR_0008, args);
		...
		if (!f.isFile()) {
			Object[] args = new Object[] { url };
			String msg = LogMessageHelper.getMsgResource().getMessage(
					LogMessageHelper.SAPR3_UR_0009, args);
			throw new ConfigurationException(msg);
		}
		...
		if (!f.canRead()) {
			Object[] args = new Object[] { url };
			String msg = LogMessageHelper.getMsgResource().getMessage(
					LogMessageHelper.SAPR3_UR_0010, args);
			throw new ConfigurationException(msg);
		}
	...
	private void initStylesheetList(String paramName)
		...
		if (paramVal != null) {
			...
			try {
				...
				try {
					...
					if (getLog() != null) {
						log.logwarn(LogMessageHelper.getMsgResource().getMessage(LogMessageHelper.SAPR3_UR_0019,new Object[] { x.getMessage() }));
					}
				}
			}
		}
		...
		if (writeStylesheetList.size() == 0) {
			...
			String msg = LogMessageHelper.getMsgResource().getMessage(
					LogMessageHelper.SAPR3_UR_0027, args);
			throw new ConfigurationException(msg);
		}
	}
	...
	private void initIterateModeStylesheets() throws ConfigurationException {
		...
		if (paramVal == null || paramVal.length() == 0) {
			...
			String msg = LogMessageHelper.getMsgResource().getMessage(
					LogMessageHelper.SAPR3_UR_0011, args);
			throw new ConfigurationException(msg);
		}
