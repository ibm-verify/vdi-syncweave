...
abstract class XslFindMethodBase implements FindMethod {
	...
	private static final String DUMMY_XML = "<sapUserXml />";
	...
	private void checkEmptyTransformResult(Result outXml)
		...
		if (outXml instanceof DOMResult) {
			...
			if (docResult == null) {
				Object[] args = new Object[] { getPostCallXsl()
						.getAbsoluteFile() };
				String msg = LogMessageHelper.getMsgResource().getMessage(
						LogMessageHelper.SAPR3_UR_0015, args);
	...
	private XmlFunctionAdapter executeFunction(String inXml,
		...
		try {
			...
			Object[] args = new Object[] { getPreCallXsl().getAbsoluteFile() };
			String msg = LogMessageHelper.getMsgResource().getMessage(
					LogMessageHelper.SAPR3_UR_0015, args);
			throw new EmptyTransformResultException(msg, x);
		}
	}
