...
final class IfrEntryImporterImpl implements IfrImporter {
	 ...
	 * 
	 * @see com.ibm.di.fc.sapr3rfc.IfrImporter#getFunctionName()
	 */
	public String getFunctionName() {
		String result = "";

		Attribute param = getEntry().getAttribute(SapR3RfcFC.PARAM_INPUT);
		if (param != null) {
	...
	public void importData(JCO.Function func) throws IfrImporterException {
		...
		Attribute attr = (Attribute) getEntry().getAttribute(
				SapR3RfcFC.PARAM_INPUT).getValue(0);
	...
	void importData(JCO.ParameterList pList, Attribute a)
	...
	void importData(JCO.Field field, Attribute a) throws IfrImporterException {
		field.setValue(a.getValue());
	}

	void importData(JCO.Record rec, Attribute a) throws IfrImporterException {
	...
	void importData(JCO.Table tab, Attribute a) throws IfrImporterException {
		int i = 0;
		Attribute itemAttr = null;
		while ((itemAttr = findChildAttr(ITEM + i, a)) != null) {
			tab.appendRow();
			importData((JCO.Record) tab, itemAttr);
			i++;
		}
	}

	void importData(JCO.Structure struct, Attribute a)
			throws IfrImporterException {
		importData((JCO.Record) struct, a);
	}
