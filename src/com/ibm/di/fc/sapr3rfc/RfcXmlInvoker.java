 ...
 * (SAP_JCO_HOME)/sapjaco.jar:(ITDI_HOME)/jars/functions/SapR3RfcFC.jar
 * -Djava.library.path=(SAP_JCO_HOME) com.ibm.di.fc.sapr3rfc.RfcXmlInvoker -f
...
public final class RfcXmlInvoker {
	...
	private static final String PROPERTIES_FILE = "saprfcfc";
	...
	public void run() {
		...
		try {
			FileReader inp = new FileReader(inXml);
			String xmlResult;
			try {
				xmlResult = sa.sendReceive(client, inp, log);
			} finally {
				inp.close();
			}
	 ...
	 * (ITDI_HOME)/_jvm/bin/java -cp
	 * (SAP_JCO_HOME)/sapjaco.jar:(ITDI_HOME)/jars/functions/SapR3RfcFC.jar
	 * -Djava.library.path=(SAP_JCO_HOME) com.ibm.di.fc.sapr3rfc.RfcXmlInvoker
