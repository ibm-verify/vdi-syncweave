...
final class IfrEntrySerializerImpl implements IfrSerializer {
	...
	public void serialize(JCO.Function func) {
		...
		Attribute respAttr = new Attribute(SapR3RfcFC.PARAM_OUTPUT);

		Attribute rfcAttr = new Attribute(getEncoder().encode(func.getName())
				+ RESPONSE_SUFFIX);
		JCO.ParameterList pList = func.getImportParameterList();
		if (pList != null) {
	...
	void serialize(Attribute a, JCO.ParameterList pList) {
	...
	void serialize(Attribute a, JCO.Field field) {
	...
	void serialize(Attribute a, JCO.Record rec) {
	...
	void serialize(Attribute a, JCO.Structure struct) {
		serialize(a, (JCO.Record) struct);
	}

	/* @modelguid {41C869FB-27EA-44EB-BD80-B516DC104A0A} */
	void serialize(Attribute a, JCO.Table tab) {
		for (int i = 0; i < tab.getNumRows(); ++i) {
			tab.setRow(i);
			Attribute newAttr = new Attribute("item" + i);
			serialize(newAttr, (JCO.Record) tab);
			a.addValue(newAttr);
		}
	}
}
