...
final class IfrXmlSerializerImpl implements IfrSerializer {
	...
	void serialize(JCoParameterList pList) {
	...
	void serialize(JCoStructure struct) {
		serialize((JCoRecord) struct);
	}

	/* @modelguid {FCB174B7-9877-4D9D-91C9-E20C970A84EA} */
	void serialize(JCoTable tab) {
		for (int i = 0; i < tab.getNumRows(); ++i) {
			tab.setRow(i);
			emmitStartTag(ITEM);
			serialize((JCoRecord) tab);
			emmitEndTag(ITEM);
		}
	}
