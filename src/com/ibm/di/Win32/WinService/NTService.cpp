...
CNTService::CNTService(const char *szServiceName, const char *szDisplayName)
{
	...
	SetServiceName(szServiceName);
	SetDisplayName(szDisplayName);
    m_iMajorVersion = 1;
    m_iMinorVersion = 0;
    m_hEventSource = NULL;

	// the service is started automaticly by default
	isAutostart = true;
	...
	if (_stricmp(argv[1], "-i") == 0) {
        ...
        if (IsInstalled()) {
            ...
            if (Install()) {
                ...
                printf("%s service failed to install. Error %d\n", m_szServiceName, GetLastError());
            }
        }
        ...
        if (!IsInstalled()) {
            ...
            if (Uninstall()) {
                ...
                printf("Could not uninstall service %s. Error %d\n", m_szServiceName, GetLastError());
            }
        }
        return TRUE; // say we processed the argument
...
void CNTService::Handler(DWORD dwOpcode)
{
    ...
    case SERVICE_CONTROL_STOP: // 1
		...
		// Mark service as stopped so the SCM can stop it for us.
		pService->SetStatus(SERVICE_STOPPED);
...
////////////////////////////////////////////////////////////////////////////////
void CNTService::SetServiceName(const char *szServiceName)
{
...
////////////////////////////////////////////////////////////////////////////////
void CNTService::SetDisplayName(const char *szDisplayName)
{
...
//
// This method can be used to unregister the event source so next time event,
// logging is needed a new event source to be registered. This is useful if
// the name of the Windows service is changed.
////////////////////////////////////////////////////////////////////////////////
void CNTService::UnregisterEventSource()
{
