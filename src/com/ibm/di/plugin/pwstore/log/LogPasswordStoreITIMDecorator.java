 ...
 * be used as the value of the password plug-in configuration property named
 * <b>syncClassname</b>.
 * </p>
 * <p>
 ...
 * PasswordStore decorated is the existing {@link LogPasswordStore}.
 * </p>
 */
public final class LogPasswordStoreITIMDecorator implements PasswordStore, IPasswordSynchronizer {
	...
	private static final String PREFIX = LogPasswordStore.PREFIX;
	...
	private static PasswordPolicyServiceBuilder getPolicyBuilder() throws ClassNotFoundException, PolicyInitializationException {
		...
		if (factClassProp == null || factClassProp.length() == 0) {
			System.setProperty(PasswordPolicyServiceBuilder.PROP_NAME_FACTORY_CLASS,
					LogPasswordStoreITIMDecorator.DEFAULT_PASSWORD_POLICY_FACTORY_CLASS);
		}
	 ...
	 */
	public LogPasswordStoreITIMDecorator() throws ClassNotFoundException, PolicyInitializationException, PolicyConnectionException,
			IllegalAccessException, InstantiationException {
		super();

		PasswordPolicyServiceBuilder builder = LogPasswordStoreITIMDecorator.getPolicyBuilder();

		factory = builder.getFactory();
		PasswordPolicyService pps = factory.newPasswordPolicyService();
		PasswordStore sync = new LogPasswordStore();
	...
	public boolean syncPassword(String id, Vector passwords) {
		return store(getPasswordChange(PasswordChange.NO_CHANGE, id, passwords, null));
	}
