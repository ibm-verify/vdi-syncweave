 ...
 */
package com.ibm.di.ui.webui.internal.templates;
...
import com.ibm.di.ui.webui.bind.BindUtil;
import com.ibm.di.ui.webui.bind.ConfigTemplate;
...
//		tslist.add(0, ct);
	...
	public String createNewTemplate(@Context HttpServletRequest req, InMultiPart inMP) throws Exception {
		...
		buf.append("<html>");
		buf.append("<head>");
		buf.append("<title>TDI Upload Solution</title>");
		buf.append("</head>");
		buf.append("<body>");
		try {
	...
	private void _createNewTemplate(@Context HttpServletRequest req, InMultiPart inMP) throws Exception {
		...
		try {
			...
			if (!file.delete())
				SystemFunctions.doNothing();
			throw new Exception("The submitted file is not a recognized TDI configuration file", e);
		}
