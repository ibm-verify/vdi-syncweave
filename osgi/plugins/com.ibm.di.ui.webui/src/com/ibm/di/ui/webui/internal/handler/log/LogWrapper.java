...
import com.ibm.di.ui.webui.bind.Logsearchresult;
...
public class LogWrapper {
	...
	public Logsearch search(Session session) throws Exception {
		...
		if(search.getConfig() == null && search.getAssemblyline() == null) {
			...
			if(search.getLogfile() != null && search.getLogfile().indexOf("/") != -1)
				inp = new Logfile(search.getLogfile(), pattern, start, lines);
			else
			...
			if(subsearch == null || str.toLowerCase().contains(subsearch)) {
				...
				if(m.matches()) {
					...
					if(fSource != -1) {
						...
						if(search.getSource() != null && !search.getSource().equals(ls.getSource())) {
							continue;
						}
