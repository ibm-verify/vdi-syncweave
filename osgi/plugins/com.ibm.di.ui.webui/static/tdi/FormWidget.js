require({cache:{
'url:tdi/templates/FormWidget.html':"<div class=\"dijitReset\" style=\"min-width: 400px; xmax-height: 600px; xoverflow:auto; background-color: white\">\n\n\t<div dojoAttachPoint=\"frmToolbar\" dojoType=\"dijit.Toolbar\" style=\"display:none;\">\n\t\t<button dojoType=\"dijit.form.Button\" dojoAttachPoint=\"okButton\" dojoAttachEvent=\"onClick:closeForm\">${nls.WebCE_more}</button>\n\t\t<button dojoType=\"dijit.form.Button\" dojoAttachPoint=\"resetButton\" dojoAttachEvent=\"onClick:querySchema\">${nls.WebCE_discover}</button>\n\t\t<button dojoType=\"dijit.form.Button\" dojoAttachPoint=\"testButton\" dojoAttachEvent=\"onClick:testConnection\">${nls.ConfigSettingsEditor_TestConnection}</button>\n\t</div>\n\t<div data-dojo-type=\"dijit.form.Form\" data-dojo-attach-point=\"FormObject\" class=\"dijitReset tdiForm\">\n\t<table width=\"90%\" style=\"margin:0; padding:0\" cellspacing=\"5px\">\n\t\t<tbody dojoAttachPoint=\"GeneralTable\"></tbody>\n\t</table>\n\t</div>\n</div>"}});
define("tdi/FormWidget", [
	"dojo/_base/declare",
	"dojo/_base/array",
	"dojo/_base/lang",
	"dojo/aspect",
	...
	"tdi/tdiapi",
	"tdi/tdiconfig",
	"tdi/ExpressionEditor",
	"tdi/NlsMixin",
	"tdi/forms/_FormWidgetMixin",
	"dojo/text!./templates/FormWidget.html",
	"tdi/tdiutil"
], function(declare, array, lang, aspect, domClass, Widget, TemplatedMixin, WidgetsInTemplate, Dialog, Toolbar, TitlePane, Button, CheckBox, FilteringSelect, SimpleTextArea, Textarea, ContentPane,
		tdiapi, tdiconfig, ExpressionEditor, tdiNlsMixin, FormWidgetMixin, template, tdiutil) {
...
//		queryButtonId: 1,
		...
		// -- fields to auto-toggle enabled/disabled
		autoToggle: {},
		
		// customScripts: Object
		//		Object where key matches a parameter, the value is a function to execute
		//		Functions in this object overrides the default chain of function calls
		customScripts: null,
		
		// includeExpressionEditor: Boolean
		//		Include expression editor if true
		includeExpressionEditor: false,
		...
		setParamValue : function(param, value) {
			...
			if(this.config != null) {
				...
				var curval = this.config.getParam(param);
				if(value != curval) {
					this.config.setParam(param, value, prot);
					
					// -- Toggle items?
					this.autoToggleItems(param, value);
					
					this.onModify(param, value);
				}
			}
		...
		autoToggleItems: function(param, value) {
			...
			if(cust) {
				array.forEach(cust.enableItems, function(key) {
					cust.tr.style.display = value ? "" : "none";
...
//					}
		...
		createControl: function(p, value) {
			...
			if(this.formData.javaClass == "com.ibm.di.connector.AssemblyLineConnector" && 
					this.formData.javaClass == "com.ibm.di.fc.AssemblyLineFC" && p.key == "operationInit") {
				var fields = [];
			...
			if(FormWidgetMixin.hasCustomControlFor(p.type)) {
			...
			} else if(FormWidgetMixin.hasCustomControlForParam(p, this.formData)) {
				...
				control = new CheckBox({readOnly:readOnly, checked:checked, name:p.key, label:tdiutil.getFormLabel(p)});
			} else if(p.type == "textarea") {
				control = new dijit.form.SimpleTextarea({readOnly:readOnly, value:value, rows:"4", style:"width:98%", name:p.key});
		...
		createFormField : function(p, hideNullValues) {
			...
			var pos = "last";
			if(this.simpleSecctions) {
				pos = target.nodeName == "TBODY" ? "last" : "after";
			}
			
			var tr = dojo.create("tr", null, target, pos);
			domClass.add(tr, "tdiFormRow");
			
			// -- If we have autotoggle then save TR handle so we can display:none it
			if(this.autoToggle[p.key]) {
				this.autoToggle[p.key].tr = tr;
			}
			
			var label = null;
			// -- idx CheckBox has label support
			if(p.type != "boolean") {
				label = this.createLabelFor(control ? control.id : "", tdiutil.getFormLabel(p));
			}
			
			var exprEditor = null;
			if(this.includeExpressionEditor) {
				//
				// -- Expression editor
				//
				exprEditor = new ExpressionEditor({control:control, config:this.config, param:p.key});
				control = exprEditor;
			}
			...
			if(this.verticalLayout) {
				...
				var td = dojo.create("td", {valign:"center", align:"right", style:"white-space:nowrap;"}, tr);
				dojo.place(label, td);
			...
			if(label)
				domClass.add(label, "tdiFormLabel");
			
			if(control) {
				
				// tag
				domClass.add(control, "tdiFormInputField");
				
				// add callback for change events
				if(this.isParamEditable(p.key)) {
					this.own(aspect.after(control, "onChange", function(value) {
						t.setParamValue(p.key, value);
					}, true));
...
//					dojo.style(control.domNode, "label", tooltip);
		...
		performCallback : function(item) {
			var func = this.customScripts ? this.customScripts.getScriptHandlerFor(item, this) : null;
			if(lang.isFunction(func)) {
				func(this, item);
			} else {
				func = FormWidgetMixin.getScriptHandlerFor(item, this);
				if(func) {
					func(this, item);
		...
		createSection : function(section) {
			...
			// if we use simple section then keep the TR in the general table
			// rather than creating a new content pane with its own table.
			if(this.simpleSections) {
				dojo.create("td", {colspan:"99", innerHTML:"<b>"+title+"</b>"}, tr);
				this._sections[section.id] = tr;
				...
				if(this.ignoreSections) {
					...
					if(title == "General" || title == "" || !title)
						new dijit.layout.ContentPane({content:table, style:"width:100%"}).placeAt(td);
					else 
						new dijit.TitlePane({title:title, open:section.expanded, content:table, style:"width:100%"}).placeAt(td);
		...
		appendALInitParams : function(fields) {
			...
			if(al != null) {
				...
				dojo.forEach(initParams.getNames(), dojo.hitch(this, function(name) {
					...
					var sic = tdiutil.parseALInitParam(initParams.getItem(name));
					var type = null;
					if(sic)
						type = sic.nativeSyntax;
					if(!type)
						type = "string";
		...
		createForm : function() {
			...
			if(showFields.length > 0) {
				...
				// 
				var pd = this.formData.parameterMapDescriptor.parameterDescriptor;
				var t = this;
				array.forEach(sections, function(sec) {
					...
					array.forEach(arr, function(p) {
						t.createFormField(p, t.hideNullValues);
					});
				});
