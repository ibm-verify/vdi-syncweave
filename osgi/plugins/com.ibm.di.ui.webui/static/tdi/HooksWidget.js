...
//				hp.startup();
		...
		postCreate: function() {
			...
			this.tree = new Tree({
				model:new TreeModel({store:this.createStore()}),
				showRoot:false,
				autoExpand:true,
				openOnClick:true,
			...
			var pd = new TooltipDialog({
				content:this.tree
			});
			pd.startup();
			
			var dd = new DropDownButton({
				label:"Hook Hierarchy", 
				dropDown:pd
			});
			dd.startup();
			...
			this.border.setTop(toolbar, {style:"width:100%; margin:0; padding:0"});
			
			this.hookDiv = new AccordionContainer({style:"width:100%; height:100%; margin:0; padding:0;"});
//			this.hookDiv = html.create("div", {style:"width:100%; height:100%; margin:0; padding:0; overflow:scroll"});
			this.border.setCenter(this.hookDiv);
