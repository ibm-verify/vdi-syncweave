...
define([
	...
	"tdi/NlsMixin",
	"dojo/text!./templates/LDSMapItemEditor.html"
...
return declare(
	[ _Widget, _TemplatedMixin, _WidgetsInTemplateMixin, nls ],
	{
			...
			if(item[prop] != value) {
				if(prop == "simple" && value != "") {
					if(item.name == value) {
						...
						delete item.simple;
						item.script = this.getSourceObject() + '[\"' + value + '"]';
					}
				} else if(prop == "addmodify") {
					item.add = (value == "add" || value == "both");
					item.mod = (value == "mod" || value == "both");
		...
		updateFields: function() {
			...
			if(this.item.script)
				this._script.set("value", this.item.script);
		...
		_onAddMod: function(args, arg2) {
			...
			this.item.mod = (args == "mod" || args == "both");
		},
		...
		_toggleType: function(value) {
			...
			if(!simple) {
				if(this.item.simple && !this.item.script) {
					this.updateValue(this.item, "script", this.getSourceObject() + '[\"' + this.item.simple + '"]'); 
					this._script.set("value", this.item.script);
				}
			}
		},
...
//			this._script.watch("value", lang.hitch(this, "updateValue", "script"));
		...
		_getValueAttr: function() {
			...
			if(type == "simple") {
				delete item.script;
				this.updateValue(item, "simple", this._attribute.get("value"));
			} else {
				if(item.simple)
					delete item.simple;
				this.updateValue(item, "script", this._script.get("value"));
			}
			if(this.isSimpleAssignment()) {
				item.assign = this.getSimpleAssignment();
			} else {
				item.assign = item.script;
			}
			return item;
		},
		...
		isSimpleAssignment: function() {
			if(this.item.script) {
				var arr = this.matchSimple(this.item.script);
				if(arr && arr.length == 2) {
					return true;
				}
				return false;
