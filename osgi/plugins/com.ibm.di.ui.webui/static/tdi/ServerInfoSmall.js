

define([
    "dojo/_base/declare",
	"dijit/_Widget",
	"dijit/_TemplatedMixin",
	"dijit/_WidgetsInTemplateMixin",
	"tdi/tdiatom",
	"tdi/tdiapi",
	"tdi/tdiutil",
	"dojo/text!./templates/ServerInfoSmall.html",
	"dijit/layout/BorderContainer",
	"dijit/layout/ContentPane",
	"dijit/TitlePane",
	"dojox/charting/Chart2D",
	"dojox/charting/themes/MiamiNice",
    "dojox/charting/widget/Legend",
    "dojox/charting/action2d/Tooltip",
	"tdi/NlsMixin"
], function(declare, Widget, TemplateMixin, WidgetsInTemplateMixin, tdiatom, tdiapi, tdiutil, template, BorderContainer, ContentPane, TitlePane,
		Chart2D, MiamiNice, Legend, Tooltip, NlsMixin) {
	
return declare(
	[ Widget, TemplateMixin, WidgetsInTemplateMixin ],
	{
		templateString : template,
		widgetsInTemplate : true,
		
		freeMem: null,
		maxMem: null,
		totalMem: null,
	
		_timerFunc : function() {
			dojo.when(tdiapi.getVMStatus(), dojo.hitch(this, function(data) {
				this.setSeries(data);
			}));
		},
	
		resize : function(obj) {
			if(obj && obj.h) {
				this._border.resize(obj);
				if(this._graph) {
					var box = dojo.position(this.Bottom.domNode);
					this._graph.resize(obj.w * 0.4, obj.h - (box.x + box.h + 10));
					var h = box.x + box.h + 15;
					this._graph2.resize(obj.w * 0.2, obj.h - h);
				}
			}
		},
		
		setSeries : function(data) {
			var used = {y:data.totalMemory - data.freeMemory, text:"Used"};
			var free = {y:(data.maxMemory-data.totalMemory) + data.freeMemory, text:"Available"};
			this._graph.addSeries("Memory", [used, free]);
			this._graph.render();
			
			this._graph2.addSeries("Threads", [data.activeThreads]);
			this._graph2.render();
			
			this.data = data;
			this._maxmemory.innerHTML = (data.maxMemory / (1024*1024)).toFixed() + "mb";
		},
		
		_openServerInfo : function() {
			// summary:
			//		callback function to open server info
		},
		
		memTooltip : function(o) {
			var used = this.data.totalMemory - this.data.freeMemory;
			var free = (this.data.maxMemory-this.data.totalMemory) + this.data.freeMemory;
			if(o.index == 0) {
				return (used / (1024*1024)).toFixed() + "mb used"; 
			} else {
				return (free / (1024*1024)).toFixed() + "mb available"; 
			}
		},
		
		threadsTooltip : function(o) {
			return this.data.activeThreads + " active thread(s)";
		},
		
		_createGraph : function() {
			this._graph = new Chart2D(this.Graph);
			this._graph.addPlot("default", {
		            type: "Pie"
	        }).setTheme(MiamiNice);
			this._tooltip = new Tooltip(this._graph, "default", {text:dojo.hitch(this, "memTooltip")});
	
			this._graph2 = new Chart2D(this.Graph2);
			this._graph2.addPlot("default", {
		            type: "StackedColumns",
		            markers: true,
		            lines: true,
		            gap: 5, minBarSize: 10, maxBarSize: 10
	        }).setTheme(MiamiNice);
			this._graph2.addAxis("y", {vertical: true, fixUpper: "major", includeZero: true});
			this._tooltip2 = new Tooltip(this._graph2, "default", {text:dojo.hitch(this, "threadsTooltip")});
		},
		
		destroy : function() {
			if(this.timer) {
				clearInterval(this.timer);
				this.timer = null;
			}
			this.inherited(arguments);
		},
		
		postMixInProperties: function() {
			declare.safeMixin(this, new NlsMixin());
		},
		
		postCreate : function() {
			this.inherited(arguments);
			
			this._createGraph();
			
			this.timer = setInterval(dojo.hitch(this, "_timerFunc"), 5000);
			this._timerFunc();
			
			dojo.when(tdiapi.getServerInfo(), dojo.hitch(this, function(data) {
				this._version.innerHTML = data.serverVersion;
			}));
		}
	}
	)
});
