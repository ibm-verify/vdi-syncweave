define([
	...
	"tdi/FilteredLogViewerNode",
	"tdi/tdiapi",
	"tdi/tdiutil",
	"tdi/NlsMixin",
	"tdi/atom/tdibatchevent",
	"dojo/text!./templates/FilteredLogViewer.html"
], function(declare, array, lang, _Widget, _TemplatedMixin, _WidgetsInTemplateMixin, MenuItem, ProgressBar, Toolbar, TooltipDialog, DropDownButton, HorizontalSlider, 
		CheckBox, ComboBox,	ComboButton, NumberTextBox, TextBox, Textarea, BorderContainer, ContentPane, FilteredLogViewerNode, 
		tdiapi, tdiutil, nlsmixin, tdibatchevent, template) {
	return declare(
		...
		_showDetails : function() {
			...
			for(var f in this.logfile) {
				...
				if(f == "modified")
					...
					str += "<a href='fds/log/download/" + this.logfile.path + "' target='_blank'>" + this.logfile.path + "</a>";
				else
					str += this.logfile[f];
				str += "</td></tr>";
			}
...
//			return this._polling.get("value") == "on";
		...
		_filterLog : function(data) {
			...
			dojo.forEach(data.result, dojo.hitch(this, function(msg) {
				...
				if(msg.content) {
					new FilteredLogViewerNode({logmsg:msg, typeColors:this.typeColors, showSource:showSource}).placeAt(tr);
				} else {
					...
					dojo.create("span", {style:"font:1em courier", innerHTML:msg.text}, td);
				}
		...
		logReadTimeout : function(err) {
			...
			if(err.status && err.status == "404") {
				console.log("Error from log read - stopping poll");
		...
		_CIDataReceived : function(data) {
			...
			if(dojo.isString(data)) {
				...
				var be = new tdibatchevent({event:data});
...
//							}
							...
							var tr = dojo.create("tr", null, this.table);
							var td = dojo.create("td", {valign:"top"}, tr);
							dojo.create("span", {innerHTML:event.message}, td);
							scrollIntoView = tr;
						}
					}
				}
		...
		postCreate : function() {
			...
			if(this.hideToolbar || this.url) {
				this._borderContainer.removeChild(this.contentTop);
...
//				dojo.style(this._progress.domNode, "display", "none");
			...
			if(this.url) {
				...
				this.openLogfile(null, null, null);
			}
		}
	})
});
